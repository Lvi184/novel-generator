...
现在开始写第{chapter_num}章："""
    ...
    response = client.chat.completions.create(
        ...
        temperature=config.get("temperature", 0.85),  # 提高温度增加多样性
        presence_penalty=0.3,  # 惩罚已出现的token
        frequency_penalty=0.3   # 惩罚频繁出现的token
    )
...
def main():
    ...
    # 生成第11-20章
    for chapter_num in range(start_chapter, 21):
        ...
        # 更新已使用的短语
        used_phrases = extract_used_phrases(previous_chapters[-5:])  # 只看最近5章
